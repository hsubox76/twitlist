...
export function getRef(collectionOrPath, doc) {
  ...
  if (pathParts.length % 2 === 0) {
    // document
    if (doc) {
      return firebase.firestore().collection(collectionOrPath).doc(doc);
    } else {
...
export async function getGuestList(uid, listUid) {
  ...
  let notes;
  if (properties.visibility === VISIBILITY.PUBLIC || properties.sharedWith.includes(uid)) {
...
export function getListNotes(uid, filterFn) {
  ...
  return getNotesQuery
    ...
    .catch(e => {
      throw new Error(`Error getting ${COLL.LISTS}/${uid}/${COLL.NOTES}: ${e.message}`)
    })
}
...
export function removeSharee(listUid, shareeScreenname, shareeUid) {
  ...
  const removeFromList = getRef(COLL.LISTS, listUid).update(updates)
    .catch(e => { throw new Error('Could not remove ' + listUid + ' from list')});
  const removeFromPendingShares = getRef(COLL.PENDING_SHARES).get()
    .then(doc => {
      if (doc.exists) {
  ...
  return getRef(COLL.SCREEN_NAMES, screenname)
    ...
    .then(screennameLookupDoc => {
      if (screennameLookupDoc.exists) {
        ...
        return getRef(COLL.LISTS, uid).update({
        ...
        .catch(e => {
          ...
          + `${screenname}:${screennameLookupDoc.data().uid} to ${COLL.LISTS}/${uid}`);
        })
      } else {
        ...
        const listPlaceholderAdd = getRef(COLL.LISTS, uid)
          ...
          .catch(e => {
            throw new Error(`Could not add ${screenname}:_ to ${COLL.LISTS}/${uid}`);
          })
        ...
        const pendingAdd = getRef(COLL.PENDING_SHARES, screenname)
          ...
          .then(pendingSharesDoc => {
            if (pendingSharesDoc.exists) {
              ...
              .catch(e => {
                throw new Error(`Could not add ${uid} to ${COLL.PENDING_SHARES}/${screenname}`);
              });
            } else {
              ...
              .catch(e => {
                throw new Error(`Could not create ${COLL.PENDING_SHARES}/${screenname}`);
              });
            }
          });
